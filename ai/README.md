...
오탐률은 그대로면서 전 공격유형 탐지율이 올라갔다 (학습 세션, k=5·95백분위 기준):
...
### 점수 A의 채점 범위 — 354차원 합에서 id 32칸으로 (2026-09-01)

점수 A는 원래 **354차원 재구성오차를 전부 합한** 값이었다. 그런데 Flooding·Replay 신호는
프레임별 `id_norm` 이 놓인 **32칸**에만 있어서, 354개로 합치면 희석돼 사라진다.
차원 그룹별로 잘라 재보면 분명하다 (규칙 없이 AI 단독, K=2, 헛울림 2번 이하가 되는 최저 임계값):

| 채점 범위 | 에피소드 탐지 | Flooding | Fuzzing |
|---|---|---|---|
| 354차원 전부 합 (기존) | 16/78 | **0/10** | 15/20 |
| **id 32칸 합 (채택)** | **36/78** | **10/10** | **18/20** |
| delta_t 32칸 합 | 40/78 | 10/10 | 15/20 |
| 집계 2칸 합 (구 점수 B) | 32/78 | 10/10 | 12/20 |
| 354차원 최댓값 / 상위 k 평균 | 2~10/78 | 0/10 | 0~4/20 |

**모델은 처음부터 신호를 갖고 있었고 채점이 뭉개고 있었다.** 재학습 없이 채점 범위만 바꿨다.

`delta_t 32칸`이 수치는 가장 좋지만 **채택하지 않았다.** 타임스탬프 기반이라 재생 속도가
틀어지면 무너진다. 정상 윈도우의 `delta_t` 정규화값을 강제로 이동시켜 오탐률을 측정했다:

| delta_t 값 | 354차원 합 (기존) | **id 32칸** | delta_t 32칸 | 규칙 |
|---|---|---|---|---|
| 원본 | 0.003% | **0.000%** | 0.020% | 0.000% |
| 0.469 (10ms 균등 재생) | 0.027% | **0.000%** | 99.997% | 0.000% |
| 0.600 | 1.817% | **0.000%** | 99.997% | 0.000% |
| 0.800 | 79.553% | **0.000%** | 99.997% | 0.000% |

**기존 채점은 재생 속도가 틀어지면 오탐이 79%까지 간다.** `id 32칸`은 완전히 면역이다
(`tools/README.md`의 재생 속도 함정 참고). 구현은 출력층 오차 누적 루프에서
`idx % 11 == 0 && idx < 352` 인 차원만 더하는 것이다. Flash +60 B, SRAM 변화 없음.

### k회 연속 필터와 임계값 (2026-08-31 교차검증으로 재설정)

윈도우 판정이 **K=2회 연속 양성**일 때만 경보한다. 점수 A 임계값은 **정상 검증셋 99.9백분위**다.

이전에는 `K=5 · 95백분위`였고, "오탐은 산발적이고 공격은 지속되므로 임계값을 조이는 것보다
K가 효율적"이라고 적혀 있었다. **그 비교는 Flooding·Fuzzing만 보고 Replay·Spoofing을
빼놓은 것이었다.** 미사용 평가셋 3종(`Pre_submit_D`·`Pre_submit_S`·`Fin_host_session_submit_S`)
전체로 다시 재면 결론이 달라진다.

임계값을 99.9백분위로 고정하고 K만 바꾼 결과 (공격 에피소드 78개 / 정상 15.3분).
시간 환산은 평가셋 실측 프레임 속도(2,672~2,818 fps → 윈도우 1개 11.4~12.0ms) 기준이다:

| K | 공격 에피소드 탐지 | 헛울림 | 최소 탐지 지연 |
|---|---|---|---|
| 1 | 78/78 | 87번 | 12ms |
| **2 (채택)** | **78/78** | **0번** | **24ms** |
| 3 | 64/78 | 0번 | 36ms |
...
(헛울림 수는 채택한 `id 32칸` 채점 기준. 표의 K별 탐지 수는 354차원 합 기준으로 잰 값이며
K가 클수록 Spoofing이 깎이는 경향은 채점 범위와 무관하게 동일하다.)

K=3 이상에서 깎이는 것은 **전부 Spoofing**이다(주입 밀도가 7%로 희소해 걸리는 윈도우가
드문드문 떨어져 있다). K=1은 헛울림이 87번으로 뛴다.

**K의 실제 역할은 규칙의 경계 오발동을 지우는 것이다.** `Pre_submit_S`에서 규칙이 정상 윈도우
44개에 발동하는데(`unique_id_ratio` 0.5938 vs 임계 0.625 — 32프레임에 고유 ID가 20종이어야
할 자리에 19종), 전부 고립된 단일 윈도우라 K=2면 사라진다. 기존 설명의 "산발적 AI 오탐을
거른다"보다 이쪽이 측정에 부합한다.

이전 설정(95백분위·K=5) 대비:

| | 95백분위·K=5 | **99.9백분위·K=2** |
|---|---|---|
| 공격 에피소드 탐지 | 51/78 | **78/78** |
| 놓친 27건 | 전부 Spoofing | — |
| 정상 15.2분 중 헛울림 | 2번 | **0번** |
| **AI 단독 탐지** | 16/78 | **36/78** |
| Fuzzing 윈도우 탐지 (`Pre_submit_D`) | 0.18% | **37.56%** |
| 프레임 단위 F1 (`Pre_submit_D`) | 0.404 | 0.401 |
| Flooding 첫 경보 | 60ms | **12ms** |
| Spoofing 첫 경보 | 3.3초 (잡은 것만) | **0.2초** |
| Flash / SRAM | — | **변화 없음** (상수만 변경) |

⚠️ **추론 시간은 바뀌지 않는다.** K는 추론이 끝난 뒤의 카운터 비교이고 임계값도 상수 비교라,
매 윈도우의 신경망 연산량은 동일하다. 줄어드는 것은 탐지 지연이지 추론 시간이 아니다.

⚠️ **"에피소드 탐지 78/78"은 공격 한 번에 최소 한 번 울렸다는 뜻이다.** 공격이 지속되는
동안 경보가 켜져 있는 비율은 Flooding 99.9%, Fuzzing 5.9%, Replay 7.0%, Spoofing 4.5%로
Flooding 외에는 간헐적이다. 원인은 주입 밀도이며(같은 Spoofing도 밀도 33%인
`Fin_host`에서는 경보 켜짐 100%), 판정이 아니라 윈도우 단위의 한계다.
...
⚠️ **2026-09-01 채점 범위 변경으로 이 감사의 해석이 바뀌었다.** 점수 A가 이제 `id_norm`
32칸의 재구성오차만 보므로, `id_norm`을 교란하면 점수가 직접 흔들리는 것이 **정상 동작**이다.

| 변형 (`Pre_submit_D`, k=2) | FPR | Flooding | Fuzzing | Replay | Spoofing |
|---|---|---|---|---|---|
| 원본 | 0.000% | 99.90% | 37.56% | 8.14% | 4.48% |
| id_norm 셔플 | 24.54% | 99.94% | 32.81% | 38.51% | 42.22% |
| 집계특성 2개 중립화 | 0.000% | 99.90% | 23.86% | 0.00% | 0.04% |

셔플은 윈도우 안의 ID 구성 자체를 파괴하므로, **구성을 보는 탐지기라면 이를 이상으로
판정하는 것이 옳다**(FPR 24.5%는 오작동이 아니라 그 결과다). 오히려 채점 범위를 바꾸기 전
"셔플해도 아무 변화 없음"이 문제였다 — 점수가 ID 정보를 아예 안 보고 있었다는 뜻이고,
그래서 Flooding 탐지가 0/10이었다.

Heydari 2026이 지적한 shortcut은 **"ID 값 → 라벨" 룩업**이다. 우리는 프레임 단위 분류가
아니라 32프레임 윈도우의 ID 구성 패턴을 모델링하므로 그 형태와는 다르지만,
**"ID 정보에 크게 의존한다"는 사실 자체는 이제 부정할 수 없다.** 정직하게 그렇게 쓴다.
`id_norm`을 0으로 채우는 변형은 학습에서 본 적 없는 극단값이라 해석에 쓰면 안 된다.
