...
## [FW-4] 추론 시간 실측 (계측 2026-08-27 / 실측 2026-09-01) — 완료

### 실측 결과

NUCLEO-F103RB(72 MHz, FPU 없음) · CubeIDE **Debug 구성(`-O0`)** · 윈도우 78개 · 유실 0.

| 구간 | min | max |
|---|---|---|
| `[FEAT]` `feature_extract_push` 32프레임 누적 | **2.91 ms** | **3.29 ms** |
| `[DET]` `vids_detect` 1회 | **20.30 ms** | **25.04 ms** |
| 윈도우당 합계 | **23.21 ms** | **28.33 ms** |

🔴 **원본 재생 속도를 따라가지 못합니다.** 평가셋 실측이 2,672~2,818 fps이므로 윈도우당
예산은 약 11.9 ms인데 실측은 그 **2.0~2.4배**입니다. 유실 없이 돌리려면 재생 속도를
약 1,100 fps 이하로 낮춰야 합니다.

**병목은 `[DET]`이고, 원인은 컴파일러 최적화 레벨입니다.** `dense_acc` 안쪽 곱셈 루프가
`-O0`에서 명령어 25개, `-Os`/`-O2`에서 6개입니다(MAC당 38~50 사이클 vs 12~18 사이클).
`firmware/Debug/`는 `-O0`이고 `firmware/Release/`는 `-Os`이지만 `ai_export/`를 빌드 목록에
넣고 있지 않아 현재 구성으로는 링크되지 않습니다. 개선은 향후 과제로 둡니다.

기존 추정치(MAC당 4~8사이클 가정, 2.6~5.3 ms)는 실측의 1/4~1/8로 빗나갔습니다.
그 가정이 손최적화 어셈블리 수준이었기 때문입니다.
...
| **보드 실측** | ✅ **완료 (2026-09-01)** — FEAT 2.91/3.29 ms · DET 20.30/25.04 ms |

**인용할 때는 빌드 구성을 반드시 함께 밝히세요.** 위 값은 CubeIDE Debug(`-O0`) 기준이며,
최적화 레벨이 결과를 3배 이상 좌우합니다.
